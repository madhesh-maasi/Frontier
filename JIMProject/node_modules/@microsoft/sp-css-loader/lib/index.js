    ...
    for (const message of result.messages) {
        switch (message.type) {
            ...
            case 'replacer': {
                ...
                cssText = cssText.replace(new RegExp(replacerMessage.replacementName, 'g'), `" + ${replacerMessage.replacementName} + "`);
                break;
            }
            case 'removal': {
...
const PRE_MODULE_PLUGINS = [
    cssnano_1.default({ preset: 'default' }),
    autoprefixer_1.default({ overrideBrowserslist: ['> 1%', 'ie >= 11'] })
];
    ...
    if (!processor) {
        ...
        if (generateCssClassName) {
            plugins = [
                ...
                postcss_modules_local_by_default_1.default({ mode: 'local' }),
                postcss_modules_extract_imports_1.default(),
                postcss_modules_scope_1.default({
            ...
            plugins = [...PRE_MODULE_PLUGINS, ...POST_MODULE_PLUGINS];
        }
        processor = postcss_1.default(plugins);
