...
var loadScriptWithoutEval_1 = require("./loadScriptWithoutEval");
...
var RequireJsLoader = /** @class */ (function () {
    ...
    RequireJsLoader.prototype.ensure = function (manifest, module) {
        var name = normalizeName_1.default(manifest);
    ...
    RequireJsLoader.prototype.load = function (manifest, name, globalName) {
        return this.requireLoad(normalizeName_1.default(manifest, name), globalName) /* tslint:disable-line:no-any */
            .then(function (module) {
            if (!name && manifest.loaderConfig.exportName) {
                // Only do this for the entry point, that has no name
    ...
    RequireJsLoader.prototype.loadFromFailoverPath = function (name) {
        return this.requireLoad(normalizeName_1.normalizeFailoverPathName(name));
    };
    /**
    ...
    RequireJsLoader.prototype.requireLoad = function (name, globalName) {
        ...
        try {
            return this._requirePromise(name)
                .then(function (module) {
            ...
            sp_diagnostics_1._TraceLogger.logError(telemetryConstants.loadComponentLogSource, error, RequireJsLoader._requireEventName);
        ...
        if (registryEntry) {
            for (var _i = 0, _a = registryEntry.depMaps; _i < _a.length; _i++) {
                ...
                if (this.requireContext.defined.hasOwnProperty(dependency.id) &&
                    ...
                    sp_diagnostics_1._TraceLogger.logError(telemetryConstants.loadComponentLogSource, new Error("Dependency \"" + dependency.id + "\" not found for module \"" + moduleName + "\""), RequireJsLoader._requireEventName);
    ...
    RequireJsLoader.prototype._configurePathResource = function (name, resource, manifest, pathConfig, mapConfig, shimConfig) {
        ...
        if (name === manifest.loaderConfig.entryModuleId) {
            ...
            normalizedName = normalizeName_1.default(manifest, name);
        ...
        if (existingNormalizedNameForAddress) {
            ...
            loadScriptWithoutEval_1.loadScriptWithoutEval(address);
    ...
    RequireJsLoader.prototype._configureComponentResource = function (name, moduleConfiguration, manifest, pathConfig, mapConfig, shimConfig) {
        ...
        if (resourceManifest) {
            mapConfig[name] = normalizeName_1.default(resourceManifest);
        }
        if (moduleConfiguration.failoverPath) {
            ...
            if (this._configuredFailoverPaths.indexOf(name) === -1) {
                var normalizedName = normalizeName_1.normalizeFailoverPathName(name);
    ...
    RequireJsLoader.prototype._fixUpJQueryKnownIssues = function (name, manifest, resource, resources) {
        ...
        if (name.toLowerCase() === jqueryuiString &&
            ...
            sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, "Fixing up JQueryUI path dependency for component \"" + manifest.id + "\" (" + manifest.alias + ")");
            if (resources[jqueryString]) {
                ...
                sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, "Didn't find JQuery path dependency in \"" + manifest.id + "\" (" + manifest.alias + ") to fix JQuery UI");
            }
        }
    };
    ...
    RequireJsLoader.prototype._fixWrongGlobalName = function (name, expectedName, knownBadName, expectedGlobalName, manifest, resource) {
        if ((name.toLowerCase() === expectedName && !resource.globalName) ||
            ...
            sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, "Fixing up wrong \"" + name + "\" path dependency for component \"" + manifest.id + "\" (" + manifest.alias + ")");
    ...
    RequireJsLoader.prototype._initialize = function () {
        ...
        config.onNodeCreated = function (node, c, moduleName, url) {
            if (isCorsEnabled_1.default(url)) {
    ...
    RequireJsLoader.prototype._resolveAddress = function (manifest, resourceName) {
        var address = resolveAddress_1.default(manifest, resourceName, this.useSecondaryCdn);
    ...
    RequireJsLoader.prototype._ensureProperModuleLoaded = function (moduleName, module, globalName) {
        if (globalName) {
            if (window.hasOwnProperty(globalName)) {
                ...
                var error = ErrorBuilder_1.default.buildScriptFailedToCreateGlobalError(globalName, this.getConfiguredUrl(moduleName));
                sp_diagnostics_1._TraceLogger.logError(telemetryConstants.loadComponentLogSource, error);
                // Public warning to advertise the configuration mistake, without failing the load.
                sp_core_library_1.Log.warn(telemetryConstants.loadComponentLogSource.id, error.message);
    ...
    RequireJsLoader.prototype._requireLoadForDifferentModuleId = function (moduleName, error) {
        ...
        if (!sp_core_library_1._SPKillSwitch.isActivated(sp_core_library_1.Guid.parse('5A521104-FCA6-4265-97B7-F3D955EE9923'), '04/25/2019', 'Loading SaaS solutions with RequireJS') &&
    ...
    RequireJsLoader.prototype._checkDependencies = function (manifest, name) {
        var moduleName = normalizeName_1.default(manifest, name);
        var missingDependencies = this.getMissingDependencies(moduleName);
        if (missingDependencies.length > 0) {
            return Promise.reject(ErrorBuilder_1.default.buildModuleHasFailedDependencyError(moduleName, missingDependencies.join(', ')));
        }
        return Promise.resolve();
    };
    RequireJsLoader.serviceKey = sp_core_library_1.ServiceKey.create('sp-loader:RequireJsLoader', RequireJsLoader);
