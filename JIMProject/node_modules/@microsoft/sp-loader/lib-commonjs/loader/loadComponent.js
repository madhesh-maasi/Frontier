...
var strings = tslib_1.__importStar(require("./Loader.resx"));
...
function loadComponent(manifest, moduleLoader) {
    if (!manifest) {
        return Promise.reject(new Error(sp_core_library_1.Text.format(strings.isUndefinedValidateError, 'manifest')));
    }
...
function _loadComponentRetryStrategy(manifest, currentRetryNumber, maxNumberRetries, moduleLoader) {
    if (currentRetryNumber === 1) {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(strings.loadComponentLog, manifest.id, manifest.alias, manifest.version));
    }
    else {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(strings.loadComponentRetryLog, manifest.id, manifest.alias, currentRetryNumber, maxNumberRetries));
    }
    return _loadComponentImpl(manifest, moduleLoader)
        .then(function (component) {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(strings.loadComponentEndLog, manifest.id, manifest.alias, manifest.version));
        return component;
    })
        ...
        if (currentRetryNumber < maxNumberRetries) {
            ...
            sp_diagnostics_1._TraceLogger.logError(telemetryConstants.loadComponentLogSource, new Error(sp_core_library_1.Text.format(commonStrings.loadComponentMaxRetriesError, manifest.id, manifest.alias, maxNumberRetries)));
            throw error;
        }
    });
}
...
function _loadComponentImpl(manifest, moduleLoader) {
    try {
        ...
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, error.message, LOAD_COMPONENT_IMPL_EVENT_NAME);
            ...
            if (resourceManifest) {
                ...
                if (moduleConfiguration_1.failoverPath) {
                    var dep = moduleLoader
                        ...
                        .catch(function (e) {
                        return _processLoadErrors(manifest, name_1, tslib_1.__spreadArrays([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadComponentDependencyErrorProcessors), function () {
                            return ErrorBuilder_1.default.buildLoadComponentDependencyFailoverPathError(manifest, name_1, resolveAddress_1.resolvePath(moduleConfiguration_1.failoverPath), e);
                        });
                    });
                    depPromises.push(dep);
                }
                    ...
                    var dep = ManifestStore_1.default.instance
                        ...
                        .catch(function (e) { return Promise.reject(ErrorBuilder_1.default.buildManifestNotFoundError(moduleConfiguration_1)); });
                    depPromises.push(dep);
                }
            }
        }
...
function _loadPathDependency(manifest, name, loadedPathDependencies, moduleLoader) {
    ...
    var qosMonitor = new sp_diagnostics_1._QosMonitor(telemetryConstants.loadPathDependencyQosScenarioName);
    var qosExtraData = {
        name: name,
    ...
    sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(strings.loadPathDependencyLog, name, manifest.id, manifest.alias));
    var resources = manifest.loaderConfig.scriptResources;
    var pathConfig = resources[name];
    if (!pathConfig) {
        return Promise.reject(ErrorBuilder_1.default.buildMissingPathDependencyError(manifest, name));
    }
    var loadPromise;
...
function _moduleLoaderLoadPathDependency(manifest, name, moduleLoader) {
    ...
    return moduleLoader.load(manifest, name, globalName).catch(function (e) {
        return _processLoadErrors(manifest, name, tslib_1.__spreadArrays([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadPathDependencyErrorProcessors), function () { return ErrorBuilder_1.default.buildLoadPathDependencyError(manifest, name, e); });
    });
}
/**
...
function _loadEntryPoint(manifest, moduleLoader) {
    return moduleLoader.load(manifest).catch(function (e) {
        return _processLoadErrors(manifest, manifest.loaderConfig.entryModuleId, tslib_1.__spreadArrays([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadEntryPointErrorProcessors), function () { return ErrorBuilder_1.default.buildLoadEntryPointError(manifest, e); });
    });
}
