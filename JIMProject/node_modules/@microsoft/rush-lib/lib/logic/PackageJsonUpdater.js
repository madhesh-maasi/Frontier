...
class PackageJsonUpdater {
    ...
    async doRushAdd(options) {
        const { projects, packageNames, initialVersions, devDependency, updateOtherPackages, skipUpdate, debugInstall, rangeStyles, variant } = options;
        ...
        for (const packageName of packageNames) {
            const initialVersion = initialVersions.get(packageName);
            const rangeStyle = rangeStyles.get(packageName);
        ...
        for (const project of projects) {
            const currentProjectUpdate = {
                ...
                updatePackages,
                dependencyType: devDependency ? PackageJsonEditor_1.DependencyType.Dev : undefined
            };
            ...
            if (this._rushConfiguration.ensureConsistentVersions || updateOtherPackages) {
                ...
                if (mismatches.length) {
                    if (!updateOtherPackages) {
                        const updatePackagesLiteral = Object.entries(updatePackages)
                            .map(([packageName, version]) => `${packageName}@${version}`)
                            .join(',');
                        throw new Error(`Adding '${updatePackagesLiteral}' to ${project.packageName}` +
