...
class TaskExecutionManager {
    ...
    async _executeTaskAndChainAsync(task) {
        ...
        try {
            const result = await task.runner.executeAsync(context);
            task.stopwatch.stop();
    ...
    _writeCondensedSummary(status, tasksByStatus, headingColor, preamble) {
        ...
        for (const task of tasks) {
            if (task.stopwatch && !task.runner.hadEmptyScript && task.status !== TaskStatus_1.TaskStatus.Skipped) {
...
//# sourceMappingURL=TaskExecutionManager.js.map