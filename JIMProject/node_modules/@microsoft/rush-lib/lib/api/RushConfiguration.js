...
class RushConfiguration {
    ...
    constructor(rushConfigurationJson, rushJsonFilename) {
        ...
        const rushPluginsConfigFilename = path.join(this._commonRushConfigFolder, RushConstants_1.RushConstants.rushPluginsConfigFilename);
        this.__rushPluginsConfiguration = new RushPluginsConfiguration_1.RushPluginsConfiguration(rushPluginsConfigFilename);
    ...
    getProjectLookupForRoot(rootPath) {
        ...
        if (!pathTree) {
            this._pathTrees.set(rootPath, (pathTree = new LookupByPath_1.LookupByPath()));
            for (const project of this.projects) {
                const relativePath = path.relative(rootPath, project.projectFolder);
                pathTree.setItemFromSegments(LookupByPath_1.LookupByPath.iteratePathSegments(relativePath, path.sep), project);
            }
        }
        return pathTree;
    }
