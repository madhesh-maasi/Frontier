...
const BuiltInPluginLoader_1 = require("./PluginLoader/BuiltInPluginLoader");
const AutoinstallerPluginLoader_1 = require("./PluginLoader/AutoinstallerPluginLoader");
class PluginManager {
    constructor(options) {
        var _a, _b;
        ...
        const ownPackageJsonDependencies = require('../../package.json').dependencies;
        function tryAddBuiltInPlugin(builtInPluginName) {
            const pluginPackageName = `@rushstack/${builtInPluginName}`;
            if (ownPackageJsonDependencies[pluginPackageName]) {
                builtInPluginConfigurations.push({
                    packageName: pluginPackageName,
                    pluginName: builtInPluginName,
                    pluginPackageFolder: node_core_library_1.Import.resolvePackage({
                        packageName: pluginPackageName,
        ...
        this._builtInPluginLoaders = builtInPluginConfigurations.map((pluginConfiguration) => {
            return new BuiltInPluginLoader_1.BuiltInPluginLoader({
                pluginConfiguration,
        ...
        this._autoinstallerPluginLoaders = ((_b = (_a = this._rushConfiguration) === null || _a === void 0 ? void 0 : _a._rushPluginsConfiguration.configuration.plugins) !== null && _b !== void 0 ? _b : []).map((pluginConfiguration) => {
            return new AutoinstallerPluginLoader_1.AutoinstallerPluginLoader({
                pluginConfiguration,
                rushConfiguration: this._rushConfiguration,
                terminal: this._terminal
            });
    ...
    async updateAsync() {
        ...
        for (const { autoinstaller } of this._autoinstallerPluginLoaders) {
            const storePath = AutoinstallerPluginLoader_1.AutoinstallerPluginLoader.getPluginAutoinstallerStorePath(autoinstaller);
            if (!preparedAutoinstallerNames.has(autoinstaller.name)) {
                node_core_library_1.FileSystem.ensureEmptyFolder(storePath);
    ...
    _applyPlugin(plugin, pluginName) {
        try {
            ...
            throw new node_core_library_1.InternalError(`Error applying "${pluginName}": ${e}`);
        }
    }
}
