...
const TaskExecutionManager_1 = require("../../logic/taskExecution/TaskExecutionManager");
...
const ProjectTaskSelector_1 = require("../../logic/ProjectTaskSelector");
...
class PhasedScriptAction extends BaseScriptAction_1.BaseScriptAction {
    ...
    async runAsync() {
        ...
        const phasesToRun = new Set(this._actionPhases);
        ...
        const taskExecutionManagerOptions = {
    ...
    async _runWatch(options) {
        const { taskSelectorOptions: { projectSelection: projectsToWatch }, stopwatch, terminal } = options;
        ...
        while (true) {
            ...
            const projectSelection = Selection_1.Selection.intersection(Selection_1.Selection.expandAllConsumers(changedProjects), projectsToWatch);
            const executeOptions = {
    ...
    async _runOnce(options) {
        const taskSelector = new ProjectTaskSelector_1.ProjectTaskSelector(options.taskSelectorOptions);
        // Register all tasks with the task collection
        const taskExecutionManager = new TaskExecutionManager_1.TaskExecutionManager(taskSelector.registerTasks().getOrderedTasks(), options.taskExecutionManagerOptions);
