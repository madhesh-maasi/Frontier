...
var KillSwitches_1 = tslib_1.__importStar(require("../common/KillSwitches"));
...
var WebPartLoadDataCollector_1 = tslib_1.__importDefault(require("./WebPartLoadDataCollector"));
...
var ClientSideWebPartManager = /** @class */ (function () {
    ...
    ClientSideWebPartManager._isLoadingIsolatedWebPart = function (context, siteUrl) {
        ...
        if (Flights_1.is1PIsolationEnabled() && manifest.isInternal && manifest.isolationLevel === 'DomainIsolation') {
            ...
            if (tenantName) {
                ...
                manifest.isolatedDomain = new URL(siteUrl.replace(ClientSideWebPartManager._1PIsolatedDomainRegex, tenantName[0] + "-app3bc2296eaa224ed29e1e946d05afa6a2")).hostname;
            }
            else if (pageLayoutType !== 'Isolated') {
                throw new Error("Cannot create isolated domain for: " + siteUrl);
            }
        }
    ...
    ClientSideWebPartManager.prototype.loadWebPart = function (webPartManagerContext) {
        ...
        try {
            ...
            var isACEWPManifest = context_1.manifest.id === 'f3bc67ee-015d-4678-a95f-c7e90ea62c1e';
            var isIsolatedACEWPManifest = context_1.manifest.id === 'e6b93447-8007-4f1e-934f-754a0031a769';
            var isACE = context_1.manifest.componentType === 'AdaptiveCardExtension' || ((_a = context_1.manifest.experimentalData) === null || _a === void 0 ? void 0 : _a.ACE) ||
                isACEWPManifest ||
                isIsolatedACEWPManifest;
            var isIsolated = ClientSideWebPartManager._isLoadingIsolatedWebPart(context_1, this._pageContext.site.absoluteUrl);
            if (isIsolated && !isACE) {
            ...
            if (isACE) {
                ...
                if (!isACEWPManifest && !isIsolated) {
                    // The ACE WP manifest should be preloaded at this point
                    context_1.manifest = sp_loader_1._ManifestStore.instance.tryGetManifest('f3bc67ee-015d-4678-a95f-c7e90ea62c1e');
                }
                else if (!isIsolatedACEWPManifest && isIsolated) {
                    // TODO: ensure Isolated ACE WP manifest is preloaded
                    context_1.manifest = sp_loader_1._ManifestStore.instance.tryGetManifest('e6b93447-8007-4f1e-934f-754a0031a769');
                }
                // @todo [VSO:WEX!#1076592] Clean up temporary ACE WP upgrade code
                if (context_1.webPartData) {
                    context_1.webPartData = tslib_1.__assign(tslib_1.__assign({}, context_1.webPartData), context_1.webPartData.properties['aceData']); // tslint:disable-line:no-string-literal
                }
            }
            context_1.webPartTag = ClientSideWebPartManager.createWebPartTag(context_1.manifest, context_1.instanceId);
            if (context_1.domElement && context_1.domElement.setAttribute && !KillSwitches_1.isClearLoadingIndicatorKSActivated()) {
                context_1.domElement.setAttribute('data-sp-web-part-id', context_1.manifest.id);
            }
            ...
            return new Promise(function (resolve) {
                ...
                if (!KillSwitches_1.default.isResolveRenderTimeActivated()) {
                ...
                .then(function () {
                var extraData = WebPartLoadDataCollector_1.default.collect(context_1.manifest, context_1.webPartTag, _this._pageContext);
                monitor_1.writeSuccess(extraData);
            })
                .catch(function (e) {
                if (!KillSwitches_1.isLogWebPartLoadFailureKSActivated()) {
                    var extraData = WebPartLoadDataCollector_1.default.collect(context_1.manifest, context_1.webPartTag, _this._pageContext);
    ...
    ClientSideWebPartManager.prototype.fetchWebPartManifests = function () {
        var _this = this;
        var meeDashboardEnabled = Flights_1.isMeeDashboardEnabled();
        var callGetClientSideComponentsByComponentType = Flights_1.isCallGetClientSideComponentsByComponentTypeEnabled();
        var shouldFetchACEs = (!callGetClientSideComponentsByComponentType ||
            (!Flights_1.isOptionalSupportedHostsFilteringEnabled() && !KillSwitches_1.fixACERequiringWPinSupportedHostsKSActivated())) &&
            meeDashboardEnabled;
        if (shouldFetchACEs) {
        ...
        if (!this._toolboxManifestsPromise) {
            ...
            var endpoint = void 0;
            if (callGetClientSideComponentsByComponentType && meeDashboardEnabled) {
                if (Flights_1.isOptionalSupportedHostsFilteringEnabled()) {
                    endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='" + Enums_1.SPClientSideComponentType.WebPart + "," + Enums_1.SPClientSideComponentType.AdaptiveCardExtension + "')";
                }
                else if (!KillSwitches_1.fixACERequiringWPinSupportedHostsKSActivated()) {
                    // ACEs will be fetched in another request
                    endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='" + Enums_1.SPClientSideComponentType.WebPart + "',supportedHostTypeValue=" + Enums_1.SupportedHostType.SharePointWebPart + ")";
                }
                else {
                    endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='" + Enums_1.SPClientSideComponentType.WebPart + "," + Enums_1.SPClientSideComponentType.AdaptiveCardExtension + "',supportedHostTypeValue=" + Enums_1.SupportedHostType.SharePointWebPart + ")";
                }
            }
            else {
                endpoint = '_api/web/GetClientSideWebParts';
            }
            var requestUrl_1 = sp_core_library_1.UrlUtilities.removeEndSlash(this._pageContext.web.serverRelativeUrl) + "/" + endpoint;
    ...
    ClientSideWebPartManager.prototype.getAdaptiveCardExtensionManifests = function () {
        ...
        for (var _i = 0, manifests_2 = manifests; _i < manifests_2.length; _i++) {
            ...
            if (manifest.componentType === 'AdaptiveCardExtension' || ((_a = manifest.experimentalData) === null || _a === void 0 ? void 0 : _a.ACE)) {
                result.push(manifest);
            }
        }
        return result;
    ...
    ClientSideWebPartManager.prototype.requestPropertyPaneAction = function (instanceId, propertyPaneAction, renderedByWebPart, context /* tslint:disable-line:no-any */) {
        ...
        if (this._iframedWebpartInstanceIds.has(instanceId)) {
            ...
            if (!KillSwitches_1.isOpenPropPaneInSeparateIframeKSActivated() && this._host.requestPropertyPaneAction) {
                //
    ...
    ClientSideWebPartManager.prototype._getWebPartContext = function (context) {
        ...
        if (!KillSwitches_1.useTeamsSDKFromHostedAppKSActivated()) {
            sdksAsync =
                (this._host.getAdditionalContextPropertiesAsync &&
                    this._host.getAdditionalContextPropertiesAsync()) ||
                    Promise.resolve({});
        }
        var parameters = {
            ...
            formFactor: context.formFactor || WebPartFormFactor_1.WebPartFormFactor.Standard,
            host: this._host,
    ...
    ClientSideWebPartManager.prototype._fetchAdaptiveCardExtensionManifests = function () {
        ...
        var endpoint;
        if (Flights_1.isCallGetClientSideComponentsByComponentTypeEnabled()) {
            endpoint = "_api/web/GetClientSideComponentsByComponentType(componentTypesString='" + Enums_1.SPClientSideComponentType.AdaptiveCardExtension + "',supportedHostTypeValue=" + Enums_1.SupportedHostType.Dashboard + ")";
        }
        else {
            endpoint = "_api/web/GetAdaptiveCardExtensions";
        }
        // if ACEs have already been fetched we don't have anything to do here.
        if (!this._toolboxACEManifestsPromise) {
            var requestUrl_2 = sp_core_library_1.UrlUtilities.removeEndSlash(this._pageContext.web.serverRelativeUrl) + "/" + endpoint;
            this._host.serviceScope.whenFinished(function () {
                var httpClient = _this._host.serviceScope.consume(sp_http_1.SPHttpClient.serviceKey);
    ...
    ClientSideWebPartManager.prototype._loadIsolatedWebPart = function (context) {
        ...
        var domainUrl = sp_component_base_1._ensureIsolatedDomainUrl(context.manifest.isolatedDomain, KillSwitches_1.fixIsolatedDomainsOnSubWebsKSActivated()
            ? this._pageContext.site.absoluteUrl
            : this._pageContext.web.absoluteUrl);
        return IframedWebPartControllerLoader_1.default().then(function (iframedWebPartControllerModule) {
            if (!_this._iframedWebPartController) {
    ...
    ClientSideWebPartManager.prototype._deleteWebPart = function (id) {
        ...
        if (wp && ClientSideWebPartManager._instanceOfBase(wp)) {
            sp_diagnostics_1._TraceLogger.logVerboseData({
                source: ClientSideWebPartManager._logSource,
                message: sp_core_library_1.Text.format(strings.WebpartDeleteRequested, ClientSideWebPartManager._getWebPartTag(wp)),
    ...
    ClientSideWebPartManager.prototype._loadLegacyFabricCssIfRequired = function (context) {
        ...
        return sp_component_base_1._LegacyThirdPartyFabricCoreLoader
            ...
            if (KillSwitches_1.default.isRejectLegacyFabricErrorKSActivated()) {
