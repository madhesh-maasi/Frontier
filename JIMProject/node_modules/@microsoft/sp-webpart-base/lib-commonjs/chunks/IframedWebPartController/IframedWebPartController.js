...
var IframedWebPart = /** @class */ (function () {
    function IframedWebPart(_controller, serviceScope, metadata) {
        ...
        if (!KillSwitches_1.isSupportThemesInIsolatedWebPartKSActivated()) {
            this._themeProvider.themeChangedEvent.add(this, this._onThemeChanged);
        }
    }
    Object.defineProperty(IframedWebPart.prototype, "componentId", {
        get: function () {
    ...
    IframedWebPart.prototype.dispose = function () {
        if (!KillSwitches_1.isSupportThemesInIsolatedWebPartKSActivated()) {
            this._themeProvider.themeChangedEvent.remove(this, this._onThemeChanged);
        }
        this.isDisposed = true;
    };
    IframedWebPart.prototype._onThemeChanged = function (eventArgs) {
        if (eventArgs.theme) {
...
var IframedWebPartController = /** @class */ (function () {
    ...
    IframedWebPartController.prototype.deleteWebPart = function (instanceId) {
        if (!KillSwitches_1.isIsolatedWebPartParentHostLifeCycleKsActivated()) {
    ...
    IframedWebPartController.prototype.loadWebPart = function (domainUrl, pageContext, webPartManagerContext) {
        ...
        var metadata = {
            ...
            theme: KillSwitches_1.isSupportThemesInIsolatedWebPartKSActivated()
                ? this._tryGetTheme(webPartManagerContext.instanceId)
                : undefined
        };
        ...
        if (!this._iframedWebparts.has(webPartManagerContext.instanceId)) {
            ...
            this._iframedWebparts.set(webPartManagerContext.instanceId, new IframedWebPart(this, webPartManagerContext.serviceScope || this._host.serviceScope, metadata));
        }
        if (sp_loader_1._SPLoaderFlights._isSafariIsolationEnabled()) {
            ...
            if (!KillSwitches_1.isIsolatedWebPartTabStopKSActivated()) {
                iframe_1.tabIndex = 0;
            }
        ...
        switch (propertyPaneState) {
            ...
            case 'OpenDetails':
                ...
                if (!this._ppIframeElement) {
                    ...
                    this._createPropertyPaneElement(instanceId, !KillSwitches_1.isOpenPropPaneInSeparateIframeKSActivated() && propertyPaneState === 'OpenDetails');
                }
    ...
    IframedWebPartController.prototype.setTheme = function (theme, instanceId) {
        ...
        var metadata = this._iframedWebparts.get(instanceId);
        if (metadata.theme && sp_lodash_subset_1.isEqual(metadata.theme, theme)) {
    ...
    IframedWebPartController.prototype._generateIframeSrcUrl = function (domainUrl, pageContext, webPartManagerContext) {
        ...
        if (webPartManagerContext.addedFromPersistedData) {
            iframeSrcUrl += ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.List + "=%7B";
            iframeSrcUrl += ((_a = webPartManagerContext.isolatedItemContext) === null || _a === void 0 ? void 0 : _a.list) || pageContext.list.id;
            iframeSrcUrl += "%7D&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.Id + "=";
            iframeSrcUrl += ((_b = webPartManagerContext.isolatedItemContext) === null || _b === void 0 ? void 0 : _b.id) || pageContext.listItem.id;
            iframeSrcUrl += "&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.WebPartInstanceId + "=";
            iframeSrcUrl += webPartManagerContext.instanceId;
        }
        else {
            iframeSrcUrl += ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.NewWebPartInstanceId + "=";
            iframeSrcUrl += webPartManagerContext.instanceId;
        }
        iframeSrcUrl += "&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.ParentWindowOrigin + "=";
        iframeSrcUrl += window.location.origin;
        iframeSrcUrl += "&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.ComponentId + "=";
        iframeSrcUrl += webPartManagerContext.manifest.id;
        iframeSrcUrl += "&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.HostedInCanvas;
        if (sp_core_library_1._BrowserUtilities.isTeamsHosted()) {
            iframeSrcUrl += "&" + ReservedIframeQueryParamKeys_1.ReservedIframeQueryParamKeys.HostedInTeams;
        }
        return iframeSrcUrl;
    };
    /**
    ...
    IframedWebPartController.prototype._createPropertyPaneElement = function (instanceId, openDetails) {
        ...
        this._iframeSrcUrl += "&" + (!KillSwitches_1.isOpenPropPaneInSeparateIframeKSActivated() && openDetails
            ...
            if (currentOrigin.toLowerCase() === event.origin.toLowerCase()) {
                ...
                switch (eventData_1.action) {
                    ...
                    case IIframedWebPartMessage_1.IframedWebPartAction.GetOboToken:
                        ...
                        var tokenProvider = sp_http_1._AadTokenProviders.configurable;
                        var tokenPromise = void 0;
                        if (application === sp_http_1._AadConstants.PRE_AUTHORIZED_APP_PRINCIPAL_ID) {
                    ...
                    case IIframedWebPartMessage_1.IframedWebPartAction.SetDimensions:
                        ...
                        if (Flights_1.isIsolatedAceEnabled()) {
                            ...
                            if (isFullScreen) {
                                ...
                                if (iframeWebPartElement.parentElement) {
                                    if (this._isHostDashboardWebPart(instanceId)) {
                                        ...
                                        iframeWebPartElement.parentElement.style.height = clientRect.height + 3 + "px";
                                    }
                                    else {
                                        // The Canvas does not hold the width so we set the parent element to the current width as a "placeholder"
                                        iframeWebPartElement.parentElement.style.width = clientRect.width + "px";
                                    }
                                }
                                // Notify the iframe of the old client size
                                // In the case of ACEs, we use this info to place the ACE into its original position
                                this._getCurrentIframeWebPartContentWindow(metadata.webPartElementId).postMessage({ instanceId: eventData_1.instanceId, clientRect: clientRect }, metadata.url);
                            }
                            else {
                                ...
                                if (height) {
                                    iframeWebPartElement.style.height = height + "px";
                                }
                                if (width && !this._isHostDashboardWebPart(instanceId)) {
                                    iframeWebPartElement.style.width = width + "px";
                                }
                            }
                        }
                        else {
                            sp_core_library_1.Validate.isNotNullOrUndefined(eventData_1.height, 'height');
                            iframeWebPartElement.style.height = eventData_1.height + "px";
                        }
                        break;
    ...
    IframedWebPartController.prototype._updateParentHost = function (_a) {
        ...
        switch (lifeCycleMethod) {
            ...
            case 'onBeforeWebPartInitialize':
                if (KillSwitches_1.default.isTeamsContextKSActivated()) {
    ...
    IframedWebPartController.prototype._showPropertyPane = function (webPartInstanceId, action) {
        ...
        if (webPartInstanceId && this._iframedWebparts.has(webPartInstanceId)) {
            ...
            var openDetailsPropertyPane = !KillSwitches_1.isOpenPropPaneInSeparateIframeKSActivated() && propertyPaneAction === 'OpenDetails';
    ...
    IframedWebPartController.prototype._setOboToken = function (instanceId, oboToken) {
        ...
        var metadata = this._iframedWebparts.get(instanceId);
        this._getCurrentIframeWebPartContentWindow(metadata.webPartElementId).postMessage(message, metadata.url);
    };
    /**
