                ...
                if (metadata.customMetadata) {
                    ...
                    if (metadata && metadata.customMetadata) {
                        for (var propName in metadata.customMetadata) {
                            ...
                            var customMetadataPropPath = metadata.customMetadata[propName];
                            if (iteration !== undefined && customMetadataPropPath.indexOf('*') !== -1) {
                                // Custom metadata should be resolved with current iteration of wildcard
                                customMetadataPropPath = customMetadataPropPath.replace('*', iteration.toString());
                            }
                            _this._validateAndIteratePath(customMetadataPropPath, properties, function (fixedPropPath, customIteration) {
    ...
    PropertyMetadataProcessor._forEachPropertyWithMetaData = function (propertiesMetadata, properties, metadataProcessor) {
        if (propertiesMetadata) {
            var _loop_1 = function (propPath) {
                /* tslint:disable-line:forin */
                var metadata = propertiesMetadata[propPath];
            ...
            var this_1 = this;
            for (var propPath in propertiesMetadata) {
                _loop_1(propPath);
            }
        }
    };
