...
function normalizeSolutionDefinition(terminal, options, manifests) {
    ...
    if (options.solution && options.solution.features) {
        for (const feature of options.solution.features) {
            ...
            feature.assets.elementFiles = appendBasePathToFeatureFiles(feature.assets.elementFiles, options.paths.sharepointAssetDir);
            feature.assets.elementManifests = appendBasePathToFeatureFiles(feature.assets.elementManifests, options.paths.sharepointAssetDir);
            feature.assets.upgradeActions = appendBasePathToFeatureFiles(feature.assets.upgradeActions, options.paths.sharepointAssetDir);
        }
    }
    else {
