 ...
 */
function readResources(terminal, options) {
    return getResourceFilePaths(terminal, options.paths.featureXmlDir, '**/Resources.resx').then((defaultResourcePaths) => {
        if (defaultResourcePaths.length > 1) {
            throw new Error('More than one default resources file');
        }
        return getResourceFilePaths(terminal, options.paths.featureXmlDir, '**/Resources.??-??.resx').then((localizedResourcePaths) => {
            return {
...
function getResourceFilePaths(terminal, featureXmlDir, globs) {
    return getFiles_1.getRelativeFilePaths(terminal, featureXmlDir, globs, 'Found resources file: ');
}
