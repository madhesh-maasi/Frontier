 ...
 */
function readCustomFeatures(terminal, options) {
    return getCustomFeatureXmlFilenames(terminal, options.paths.featureXmlDir).then((filenames) => {
        return getCustomFiles(terminal, options.paths.featureXmlDir).then((files) => {
            return {
                files: files,
...
function getCustomFeatureXmlFilenames(terminal, featureXmlDir) {
    return getFiles_1.getRelativeFilePaths(terminal, path.join(featureXmlDir, constants_1.default.RelsFolder), `*${constants_1.default.RelsXml}`, 'Found custom feature file: ').then((filenames) => {
...
function getCustomFiles(terminal, featureXmlDir) {
    return getFiles_1.getFiles(terminal, featureXmlDir, '**/*.*', 'Found custom file: ');
}
