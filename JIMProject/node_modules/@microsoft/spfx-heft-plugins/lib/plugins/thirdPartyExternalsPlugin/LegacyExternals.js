...
const ESCAPED_TOKEN_REGEXP = new RegExp(lodash_1.escapeRegExp(lodash_1.escapeRegExp(LOCALE_TOKEN)));
const ESCAPED_TOKEN_GLOBAL_REGEXP = new RegExp(lodash_1.escapeRegExp(lodash_1.escapeRegExp(LOCALE_TOKEN)), 'g');
...
class LegacyExternals {
    constructor(options) {
        ...
        this._singleLocale = options.singleLocale;
        ...
        for (const plugin of webpackConfiguration.plugins) {
            if (plugin instanceof ManifestPlugin_1.ManifestPlugin) {
                plugin._options.tryGetLegacyExternalByNameAsync = async (name) => {
                    return await this._tryGetLegacyExternalsByName(name);
                };
    ...
    async _ensureLocalizedResourcesAsync() {
        if (!this._discoveredLocalizedResources) {
            const localizedResourceKeys = Object.keys(this._configJson.localizedResources || {});
            this._terminal.writeVerboseLine(`${localizedResourceKeys.length} localized resources defined.`);
            this._discoveredLocalizedResources = new Map();
            for (const [resourceId, pattern] of Object.entries(this._configJson.localizedResources)) {
                let result = await this._collectResourcesForKeyAsync(pattern);
                if (this._singleLocale) {
                    const singleLocaleData = result[this._singleLocale];
                    if (!singleLocaleData) {
                        throw new Error(`The selected locale (${this._singleLocale}) is not present in the localized data for resource ` +
                            `"${resourceId}".`);
                    }
                    else {
                        result = {
                            [this._singleLocale]: singleLocaleData
                        };
                    }
                }
        ...
        if (!pattern.match(LOCALE_TOKEN_REGEXP)) {
            ...
            if (!files || files.length === 0) {
                ...
                const regexPattern = new RegExp(lodash_1.escapeRegExp(normalizedPattern)
                    ...
                    // this lint rule actually has a bug, so we'll remove this exception when it gets fixed. VSO#513580
    ...
    async _tryGetLegacyExternalsByName(externalName) {
        ...
        if (!result) {
            ...
            if (localizedFiles) {
                ...
                for (const [localeName, filePath] of Object.entries(localizedFiles)) {
                    ...
                    if (!this._serveMode && this._production) {
                        const fileHash = getFullHash_1.getFullHash(fileContents);
            ...
            else if (this._configJson.externals && this._configJson.externals[externalName]) {
                const external = this._configJson.externals[externalName];
                result = {
                    moduleConfigurations: {},
                    assets: {}
                };
                const includeExternal = async (name, externalModule) => {
                    let externalPath;
                    ...
                    if (typeof externalModule === 'string') {
                        ...
                        if (externalModule.globalDependencies) {
                            await Promise.all(externalModule.globalDependencies.map(async (dependencyName) => {
                                const dependency = this._configJson.externals[dependencyName];
                                if (!dependency) {
                                ...
                                await includeExternal(dependencyName, dependency);
                            }));
                        }
                    }
                    let assetLoadPath;
                ...
                await includeExternal(externalName, external);
            }
            if (result) {
