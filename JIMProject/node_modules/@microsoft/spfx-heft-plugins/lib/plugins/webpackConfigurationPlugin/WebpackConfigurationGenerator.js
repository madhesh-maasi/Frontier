...
const module_minifier_plugin_1 = require("@rushstack/module-minifier-plugin");
...
const hashed_folder_copy_plugin_1 = require("@microsoft/hashed-folder-copy-plugin");
// Local
    ...
    static async generateWebpackConfigurationAsync(options) {
        ...
        const config = {
            ...
            plugins: [
                ...
                new hashed_folder_copy_plugin_1.HashedFolderCopyPlugin(),
                new WebpackStatsPlugin_1.WebpackStatsPlugin({
        ...
        let localizedData = {};
        let selectedLocales = [constants_1.DEFAULT_LOCALE];
        if (options.getLocalization) {
            ...
            if (localizedDataKeys.length > 0) {
                if (options.singleLocale) {
                    if (localizedData[options.singleLocale] || options.singleLocale === constants_1.DEFAULT_LOCALE) {
                        // If the single locale we're building is the default locale, we just want to use the
                        // strings in the src folder
                        selectedLocales = [options.singleLocale];
                    }
                    else {
                        throw new Error(`The selected locale (${options.singleLocale}) is not present in the localized data.`);
                    }
                }
                else if (options.production) {
                    selectedLocales = localizedDataKeys;
                }
            }
        ...
        if (options.production) {
            ...
            config.optimization.minimizer = [
                new module_minifier_plugin_1.ModuleMinifierPlugin({
                    minifier: new module_minifier_plugin_1.WorkerPoolMinifier({
