...
class CumulativeManifestProcessor {
     ...
     */
    async generateCumulativeManifest(debugManifests) {
        if (!this._options) {
        ...
        const manifestsFileBuilder = new ManifestsFileBuilder_1.ManifestsFileBuilder(this._options);
        await manifestsFileBuilder.buildManifestsJs(manifestsArray, path.join(this._options.rootPath, this._options.tempFolderName, `${CUMULATIVE_MANIFEST_FILENAME}.js`));
    }
    reset() {
    ...
    _insertManifestIntoMap(referencedProjectManifests, manifestPath, packageName, fileModifiedTime, manifest, isAssembly) {
        ...
        if (!existingReference ||
            existingReference.manifestCreationTime > existingReference.manifestCreationTime) {
    ...
    _doTemporaryManifestFixups(manifestData) {
        function trimLeadingDist() {
            ...
            else if (typeof entryModule.path !== 'string') {
                throw new Error('Expected to find a string path');
            }
            else if (entryModule.path.startsWith(DIST_SLASH)) {
                entryModule.path = entryModule.path.substr(DIST_SLASH.length);
            }
        }
    ...
    _resolvePackage(packageName, rootPath, resolveSymlinks) {
        ...
        try {
            return path.dirname(Resolve.sync(packageName, {
