...
class AzureUploader {
    constructor(options) {
        ...
        const storageAccountUrl = AzureUploader.getStorageAccountUrl(options.storageAccountName);
    ...
    static getStorageAccountUrl(storageAccountName) {
     ...
     */
    async uploadFilesToAzureAsync(terminal, files, maxParallelism = AzureUploader.DEFAULT_AZURE_MAX_PARALLELISM, maxRetries = AzureUploader.DEFAULT_AZURE_MAX_NUMBER_OF_RETRIES, retryDelayInMs = AzureUploader.DEFAULT_AZURE_RETRY_DELAY_MILLISECONDS, onFileUploaded) {
        ...
        await node_core_library_1.Async.forEachAsync(files, async (file) => {
            await RetryUtilities_1.RetryUtilities.runWithRetriesAsync(() => this.uploadFileToAzureAsync(childTerminal, file.localPath, file.azurePath, file.blobHeaders), maxRetries, retryDelayInMs);
