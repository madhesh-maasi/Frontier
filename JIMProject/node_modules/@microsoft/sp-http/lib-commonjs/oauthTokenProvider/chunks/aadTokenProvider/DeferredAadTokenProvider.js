...
var strings = tslib_1.__importStar(require("../../../SPHttpStrings.resx"));
...
var DeferredAadTokenProvider = /** @class */ (function () {
     ...
     * @internal
     */
    function DeferredAadTokenProvider(tokenAcquisitionEvent, beforeRedirectEvent, configuration, oboConfiguration, _oboFirstPartyTokenCallback, _oboThirdPartyTokenCallback) {
     ...
     * @internal
     */
    DeferredAadTokenProvider.prototype._getTokenData = function (resourceEndpoint, useCachedToken, skipLogging) {
        if (useCachedToken === void 0) { useCachedToken = true; }
        ...
        if (this._shouldUseMsalTokenProvider()) {
            return this._getMsalTokenProvider(this._defaultConfiguration).then(function (tokenProvider) {
                return tokenProvider.getTokenData(resourceEndpoint, useCachedToken, skipLogging);
            });
        }
        else {
    ...
    DeferredAadTokenProvider.prototype._getTokenInternal = function (resourceEndpoint, configuration, useCachedToken) {
        ...
        if (this._shouldUseOboTokenExchange() && this._oboConfiguration) {
            ...
            getTokenPromise = this._getOboTokenProvider(sp_core_library_1._SPFlight.isEnabled(60254) /* Support 1st party OBO */
                ? tokenProviderConfig
                : this._defaultConfiguration, this._oboConfiguration, this._oboFirstPartyTokenCallback, this._oboThirdPartyTokenCallback).then(function (tokenProvider) { return _this._getToken(tokenProvider, resourceEndpoint, useCachedToken); });
        }
        ...
        return getTokenPromise
            .then(function (token) {
            if (AadKillSwitches_1.AadKillSwitches.isLogAadAdditionalTelemetryEnabled()) {
    ...
    DeferredAadTokenProvider.prototype._addFailedRequest = function (resourceEndpoint) {
        if (AadKillSwitches_1.isSkipKnownErroneousResourcesEnabled()) {
     ...
     * MSAL should be used when either conditions are true:
     *   1. AAD Session ID is a valid GUID.
     *   2. The flight removing Implicit Token Provider is active.
    ...
    DeferredAadTokenProvider.prototype._shouldUseMsalTokenProvider = function () {
        var isAadSessionIdValidGuid = !!sp_core_library_1.Guid.tryParse(this._defaultConfiguration.aadSessionId);
        var shouldUseMsalTokenProvider = isAadSessionIdValidGuid || AadFlights_1.AadFlights.isRemoveLegacyAuthProvidersFlightEnabled();
        if (!isAadSessionIdValidGuid) {
            if (shouldUseMsalTokenProvider) {
                this._defaultConfiguration.aadSessionId = '';
            }
            var aadSessionIdQos = new sp_diagnostics_1._QosMonitor('DeferredAadTokenProvider._shouldUseMsalTokenProvider');
            var extraQosData = {
                aadSessionId: this._defaultConfiguration.aadSessionId
            };
    ...
    DeferredAadTokenProvider.prototype._getMsalTokenProvider = function (configuration) {
        var _this = this;
        return Promise.resolve().then(function () { return tslib_1.__importStar(require('../msalImplicit/MsalTokenProvider' /* webpackChunkName: 'msalImplicit' */)); }).then(function (msalTokenProviderModule) {
    ...
    DeferredAadTokenProvider.prototype._getOboTokenProvider = function (configuration, oboConfiguration, oboFirstPartyTokenCallback, oboThirdPartyTokenCallback) {
        ...
        return this._getAdalAuthContextManager(configuration).then(function (adalAuthContextManager) {
            if (AadKillSwitches_1.AadKillSwitches.isHaveFirstAndThirdOboProvidersEnabled() &&
                _this._isFirstParty(configuration.servicePrincipalId)) {
    ...
    DeferredAadTokenProvider.msalTrialRunKillSwitchGuid = sp_core_library_1.Guid.parse('fa0b23a7-7b4e-4fbe-b635-bbed397c3ef5');
